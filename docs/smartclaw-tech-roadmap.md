...
| 决策项 | 选型 | 版本/备注 | 参考来源 |
|--------|------|----------|---------|
| Agent 编排 | LangGraph StateGraph | >= 0.4 | 架构参考 PicoClaw `pkg/agent/loop.go` |
| 浏览器引擎 | Playwright + CDP 双模式 | 最新版 | 架构参考 Browser Use + OpenClaw `src/browser/` |
| 页面理解 | Accessibility Tree 为主 + 截图为辅 | - | 参考 OpenClaw `pw-role-snapshot.ts` |
| 默认 LLM | GPT-4o（LangChain 接入） | 备选 Claude Sonnet 4 | 参考 PicoClaw `pkg/providers/` |
| 多模态 | 第一阶段支持 | Vision 能力 | - |
| 记忆存储 | SQLite | LangGraph SqliteSaver | 参考 PicoClaw `pkg/memory/` |
| 向量数据库 | sqlite-vec | LangChain SQLiteVec 集成 | 参考 OpenClaw `src/memory/sqlite-vec.ts` |
| MCP SDK | 官方 mcp Python SDK | >= 2.1 | 参考 PicoClaw `pkg/mcp/manager.go` |
| MCP 传输 | stdio 为主，Streamable HTTP 为辅 | - | 参考 PicoClaw MCP 配置 |
| 配置格式 | YAML + Pydantic Settings | - | 参考 OpenClaw YAML + Zod Schema |
| 包管理器 | uv | pyproject.toml + uv.lock | - |
| Python 版本 | 3.12+ | MCP SDK 要求 | - |
| API 框架 | FastAPI | - | 参考 PicoClaw `pkg/gateway/` |
| 日志 | structlog | 结构化日志 | 参考 PicoClaw `pkg/logger/` |
| HTTP 客户端 | httpx | 异步支持 | - |
| Web 搜索 | tavily-python | 默认搜索引擎，备选 Brave/DuckDuckGo | 参考 PicoClaw `pkg/tools/search_tool.go` |
| 凭证管理 | python-dotenv + keyring | 环境变量 + 系统凭证 | 参考 PicoClaw `pkg/credential/` |
| 测试框架 | pytest + pytest-playwright | 单元测试 + 浏览器测试 | - |
| 类型检查 | mypy | 严格类型标注 | - |
| Linter | ruff | Astral 出品，替代 flake8 + black | - |
| Skills 定义 | YAML + importlib 动态加载 | 参考 PicoClaw loader.go | 参考 PicoClaw `pkg/skills/` |
| 插件体系 | pluggy / entry_points | 第二阶段 | 参考 OpenClaw `src/plugins/` Plugin SDK |
| Hook 系统 | asyncio 事件模式 | 生命周期 Hook | 参考 OpenClaw `src/hooks/` |
| 安全体系 | 分层安全（路径 + 工具 + 审计） | - | 参考 OpenClaw `src/security/` |
| Sub-Agent | LangGraph SubGraph | - | 参考 PicoClaw `pkg/agent/subturn.go` + OpenClaw `subagent-*.ts` |
| 多 Agent | LangGraph Multi-Agent | - | 参考 PicoClaw Agent 绑定机制 |
| 定时任务 | APScheduler | - | 参考 PicoClaw `pkg/cron/` |
| 可观测性 | structlog + OpenTelemetry | 可选 | 参考 OpenClaw `diagnostics-otel` |
| 工具注册 | LangChain Tools | - | 参考 PicoClaw `pkg/tools/registry.go` |
| 工具策略 | 自研 tool_policy | - | 参考 OpenClaw `tool-policy.ts` |
...
| # | 能力模块 | Python 实现方案 | 参考来源 | 优先级 |
|---|---------|----------------|---------|--------|
| 1 | LLM 接入层 | LangChain ChatModel | PicoClaw `pkg/providers/` | P0 |
| 2 | 工具调用系统 | LangChain Tools + 自定义 | PicoClaw `pkg/tools/` | P0 |
| 3 | MCP 协议支持 | mcp Python SDK >= 2.1 | PicoClaw `pkg/mcp/manager.go` | P0 |
| 4 | 推理和规划（ReAct） | LangGraph StateGraph | PicoClaw `pkg/agent/loop.go` | P0 |
| 5 | 记忆系统（基础） | LangGraph SqliteSaver | PicoClaw `pkg/memory/` | P0 |
| 6 | 配置管理 | Pydantic Settings + YAML | OpenClaw YAML + Zod Schema 思路 | P0 |
| 7 | 基础安全 | pathlib 路径限制 | PicoClaw 工具层路径验证 | P0 |
| **8** | **浏览器引擎（核心）** | **Playwright + CDP 双模式** | **Browser Use 架构 + OpenClaw `src/browser/`** | **P0** |

### 第二阶段：增强能力

| # | 能力模块 | Python 实现方案 | 参考来源 | 优先级 |
|---|---------|----------------|---------|--------|
| 9 | Skills 系统 | YAML + importlib 动态加载 | PicoClaw `pkg/skills/` | 高 |
| 10 | Sub-Agent | LangGraph SubGraph | PicoClaw `pkg/agent/subturn.go` + OpenClaw `subagent-*.ts` | 高 |
| 11 | 多 Agent 协同 | LangGraph Multi-Agent | PicoClaw Agent 绑定机制 | 中 |
| 12 | 可观测性 | structlog + OpenTelemetry | OpenClaw `diagnostics-otel` | 中 |
| 13 | RAG/知识管理 | sqlite-vec + LangChain | OpenClaw `src/memory/sqlite-vec.ts` | 中 |
| 14 | 生命周期 Hook | asyncio 事件模式 | OpenClaw `src/hooks/` | 低 |
| 15 | 评估优化 | LangGraph 条件路由 + 成本追踪 | PicoClaw `pkg/routing/` | 低 |
| 16 | 插件体系 | pluggy / entry_points | OpenClaw `src/plugins/` Plugin SDK | 低 |
| 17 | 分层安全 | 路径策略 + 工具策略 + 审计日志 | OpenClaw `src/security/` | 中 |
